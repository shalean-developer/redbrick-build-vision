...
export default async function ServiceCityPage({
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <ServiceLocationJsonLd path={path} serviceName={service.name} cityName={loc.name} faqs={money.faqs} />
      <Navbar />
      <main className="flex-grow">
        <div className="bg-gradient-to-b from-primary/10 to-background py-16 md:py-20">
          <div className="container mx-auto px-4 max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-6">{service.name} in {loc.name}</h1>
            <p className="text-lg text-muted-foreground mb-4">{service.summary}</p>
            <p className="mb-8 rounded-md border border-primary/20 bg-primary/5 px-4 py-3 text-sm text-foreground leading-relaxed">{money.pricingHook}</p>
            <div className="flex flex-wrap gap-3">
              <Button asChild size="lg"><Link href="/contact">Request a quote</Link></Button>
              <Button asChild variant="outline" size="lg"><Link href="/projects">See verified projects</Link></Button>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 py-16 max-w-3xl">
          <div className="prose prose-lg max-w-none space-y-6 text-muted-foreground">
            {intros.map((p) => <p key={p}>{p}</p>)}
          </div>

          <section className="mt-14" aria-labelledby="local-signals-heading">
            <h2 id="local-signals-heading" className="text-2xl font-bold mb-4">Local sites &amp; climate context</h2>
            <p className="text-muted-foreground leading-relaxed mb-4">{money.localSignalsParagraph}</p>
            <p className="text-muted-foreground leading-relaxed"><strong className="text-foreground">Materials &amp; systems:</strong> {money.materialsNote}</p>
          </section>

          <section className="mt-14" aria-labelledby="scope-heading">
            <h2 id="scope-heading" className="text-2xl font-bold mb-4">Typical {service.name.toLowerCase()} scopes around {loc.name}</h2>
            <p className="text-sm text-muted-foreground mb-4">Illustrative scope types we quote and plan for in the region. These are not presented as completed local projects.</p>
            <ul className="list-disc pl-6 text-muted-foreground space-y-2">{money.proofProjects.map((line) => <li key={line}>{line}</li>)}</ul>
          </section>

          <section className="mt-14 rounded-xl border bg-card p-6 shadow-card" aria-labelledby="pricing-heading">
            <h2 id="pricing-heading" className="text-2xl font-bold mb-3">{money.pricingHeading}</h2>
          ...
          <section className="mt-14" aria-labelledby="process-heading">
            <h2 id="process-heading" className="text-2xl font-bold mb-4">{money.processHeading}</h2>
            <ol className="list-decimal pl-6 text-muted-foreground space-y-3">{money.processSteps.map((step) => <li key={step}>{step}</li>)}</ol>
          </section>

          {showShaleanReferralOnMoneyPage(service.slug) ? <ShaleanCleaningReferral locationName={loc.name} serviceName={service.name} /> : null}

          <section className="mt-14 rounded-lg bg-muted p-6" aria-labelledby="trust-heading">
            <h2 id="trust-heading" className="text-2xl font-bold mb-4">Why teams brief Team Edlick</h2>
            <ul className="list-disc pl-6 text-muted-foreground space-y-2">{money.trustBullets.map((t) => <li key={t}>{t}</li>)}</ul>
          </section>
          ...
          <div className="mt-14">
            ...
            <ul className="list-disc pl-6 text-muted-foreground space-y-2">{service.bullets.map((b) => <li key={b}>{b}</li>)}</ul>
          </div>
          ...
          <div className="mt-12 p-6 rounded-lg bg-muted">
            ...
            <ul className="flex flex-wrap gap-x-4 gap-y-2 list-none pl-0">
              {otherServices.map((s) => <li key={s.slug}><Link href={`/services/${s.slug}/${loc.city}`} className="text-primary hover:underline text-sm font-medium">{s.name}</Link></li>)}
            </ul>
            <p className="mt-4 text-sm text-muted-foreground">Planning multiple trades? <Link href="/contact" className="text-primary hover:underline">Tell us the full scope on one brief</Link> — we sequence waterproofing, tiling, and finishing in the right order.</p>
          </div>

          <TopicAuthorityLinks serviceSlug={service.slug} serviceName={service.name} citySlug={loc.city} cityName={loc.name} />

          <p className="mt-10 text-muted-foreground">Hub: <Link href={`/locations/${loc.city}`} className="text-primary hover:underline">Construction overview for {loc.name}</Link>{" · "}<Link href="/blog" className="text-primary hover:underline">Guides &amp; updates</Link>.</p>
        </div>
      </main>
      <Footer />
    </div>
